...
function domToJson(dom) {
    var json = {
            totalPages: Number(dom.Pages),
            dimensions: dom.BoundingBox,
            pages: []
        },
        pagesHash = {},
        curLinks,
    ...
    //make first pass through pages to build object hash
    Object.keys(dom.pageObjects).forEach(function (hashVal) {
        pagesHash[dom.pageObjects[hashVal].objId] = hashVal;
    });

    //make an ordered second through pages and cross-reference resources
    for (curPage = 1; curPage <= dom.Pages; curPage += 1) {
        curPageObj = {
            pageId: 'page' + curPage,
            links: []
        };

        //get reference to links for current page
        curLinks = dom.pageObjects[curPageObj.pageId].links;

        if (curLinks) {
            //loop through each link and grab the referenced resource
            curLinks.forEach(function (curObjId) {
                curResource = dom.resourceObjects[curObjId];
                curPageObj.links.push({
                    hitArea: curResource.rect,
                    link: pagesHash[curResource.destination]
                });
            });
        }

        json.pages.push(curPageObj);
    }
...
function parse(inputFile) {
    ...
    var returnValue = {
        ...
        toJSON: domToJson.bind(null, psDOM)
    };
    var domPtr = psDOM;
